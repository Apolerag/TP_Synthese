...
Image *readImage( const std::string& filename )
{
    ...
    if(is_color_image(filename))
    {
        ...
        const int channels= (format.BitsPerPixel == 32) ? 4 : 3;
        image= (new Image(filename))->create(width, height, channels, Image::UNSIGNED_BYTE);
        if(image == NULL)
            return NULL;
        ...
        else if(format.BitsPerPixel == 24)
        {
            for(int y= height -1; y >= 0; y--, py++)
            {
                ...
                for(int x= 0; x < width; x++, p+= format.BytesPerPixel)
                {
                    ...
                    image->setPixel(x, y, VecColor(r / 255.f, g / 255.f, b / 255.f));
                }
            }
        }
    ...
    else if(is_hdr_image(filename))
    {
        ...
        image= (new Image(filename))->create(width, height, 3, Image::FLOAT);
        if(image == NULL)
            return NULL;
    ...
    else if(is_exr_image(filename))
    {
        ...
        image= (new Image(filename))->create(width, height, 4, Image::FLOAT);
        Rgba *p= pixels[0];
        for(int y= height -1; y >= 0; y--)
        for(int x= 0; x < width; x++, p++)
            image->setPixel(x, y, VecColor((float) p[0].r, (float) p[0].g, (float)p[0].b));
    }
#endif
...
int writeImage( const std::string& filename, Image *image )
{
    ...
    if(filename.rfind(".bmp") != std::string::npos || filename.rfind(".png") != std::string::npos )
    {
        ...
        if(filename.rfind(".bmp") != std::string::npos)
            ...
            ERROR("invalid image format. failed.\n");
